...
marker=14000 # an index for folders to save results
...
do
       ...
       do
	    mkdir /scratch/frarzani/pqc_out/epochs/${marker} # this is for cluster
            paramstr="-s true -n ${steps} -p ${points} -l tcemoidcemoidcemoidcemoidcemoid -f /scratch/frarzani/pqc_out/epochs/${marker} -ss ${ss} -re 2 -ep true -epn ${epochs} -ce ${ce} -cg ry -sq ${layout}"
...
printf "%s\n" "${parameters[@]}" > param_file_eps_symm_rand_symmetry.txt
