...
#
#SBATCH --array=0-9%10                  # launches an array of X identical jobs id 0..X, each using the allocation above, Y running concurrently
...
/usr/bin/time -f "\t%E real,\t%M kb MaxMem" python3 -u run_ttt.py -s false -n $steps -p $points -l tcemoid -f output/depth_7 -ss 0.008 -sr true -re $depth -ep true -epn $epochs