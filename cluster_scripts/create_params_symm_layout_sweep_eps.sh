...
marker=12 # an index for folders to save results
...
for ce in "true" "false"; do
    for reps in $(seq 4 1 5); do # sweeping repetitions of the base layout
        for ss in $(seq 0.008 0.002 0.01); do # sweep step size
            ...
            paramstr="-s true -n ${steps} -p ${points} -l tcemoidcemoidcemoid -f /scratch/frarzani/pqc_out/epochs/${marker} -ss ${ss} -re ${reps} -ep true -epn ${epochs} -ce ${ce} -cg cry"
