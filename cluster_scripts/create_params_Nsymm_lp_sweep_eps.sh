...
#for ce in "true" "false"; do
    for reps in $(seq 1 1 3); do # sweeping repetitions of the base layout
	    ...
	    paramstr="-s false -n ${steps} -p ${points} -l t${p} -f /scratch/frarzani/pqc_out/epochs/${marker} -ss ${ss} -re ${reps} -ep true -epn ${epochs} -cg ry"
...
printf "%s\n" "${parameters[@]}" > param_file_eps_lp_Nsymm.txt
