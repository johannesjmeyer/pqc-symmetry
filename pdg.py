...
def translate_to_parameters(design, symmetric=True):
    ...
    for i in design.replace(" ", ""):
        if i == 't':
            ...
            # entangle direction
            param_args.append(param_args[-1]+args['ed'])
        elif i == 'sd':
            # direction
            param_args.append(param_args[-1]+args['sd'])
...
def circuit(situation, params, symmetric, design="tceocem tceicem tcedcem"):
    ...
    ngame = np.pi*situation[0] # normalize entries of game so they are between -pi/2, pi/2
...
def gen_games_sample(size, output = None):
    ...
    '''
    sample = random.sample(games_data, size)
...
pdg_dev = qml.device("default.qubit", wires=11)
...
class road_detection():
    ...
    def run_epochs(self, epochs, samplesize_per_step, steps_per_epoch, stepsize, multiplicator=1, data_file = None):
        ...
        self.test_batch = gen_games_sample(200)[0]
    ...
    def check_accuracy(self, check_size=100, check_batch=None):
        ...
        for i, situation in enumerate(check_batch):
            ...
            result = full_circ_torch(situation, self.thetalm, self.symmetric, self.design).detach().numpy()
    ...
    """
    def check_accuracy_old(self, check_size = 100): # TODO: check if accuracy varies for same run
    ...
    def save(self, name, exec_time=0):
        ...
        to_save = {'symmetric': self.symmetric,'accuracy': self.confusion_matrix,'execution time': exec_time, 'steps': self.steps, 'stepsize': self.stepsize, 'design': self.design, 'cost function': self.gd_cost, 'sample size': self.sample_size, \
