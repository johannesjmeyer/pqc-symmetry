...
marker=11000 # an index for folders to save results
...
do
       ...
       do
	    mkdir /scratch/frarzani/pqc_out/pdg/${marker} # this is for cluster
            paramstr="-s false -n ${steps} -p ${points} -l t${layout} -f /scratch/frarzani/pqc_out/pdg/${marker} -ss ${ss} -re ${reps} -cg rz"
...
printf "%s\n" "${parameters[@]}" > param_file_pdg_Nsymm_z_rand.txt
