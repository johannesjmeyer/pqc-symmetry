...
marker=3000 # an index for folders to save results
...
#for ce in "true" "false"; do
    for reps in $(seq 1 1 5); do # sweeping repetitions of the base layout
	    ...
	    paramstr="-s false -n ${steps} -p ${points} -l t${p} -f /scratch/frarzani/pqc_out/epochs/${marker} -ss ${ss} -re ${reps} -ep true -epn ${epochs} -ce ${ce}"
